...
export function encodeDirectoryRecord(input: DirectoryRecordInput): Uint8Array {
  ...
  writeUint16Both(bytes, 28, checkedVolumeSequenceNumber(input.volumeSequenceNumber ?? 1));
...
function checkedVolumeSequenceNumber(value: number): number {
  if (!Number.isInteger(value) || value < 1 || value > 0xffff) {
    throw new RangeError("volume sequence number must be an integer from 1 to 65535");
  }
  return value;
}
