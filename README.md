...
Generated descriptor sequences contain one Primary Volume Descriptor. The reader accepts one or more Primary Volume Descriptors for interoperability, followed by zero or more Supplementary Volume Descriptors, zero or more Enhanced Volume Descriptors, zero or more Volume Partition Descriptors, zero or more Boot Records, and one or more Volume Descriptor Set Terminators.
...
`parseIsoImage(image)` includes regular file payloads, volume partition payloads, and El Torito boot image payloads by default. Use `parseIsoImage(image, { includeData: false })` to read descriptors and directory trees without loading those payload bytes. Images with multiple Primary Volume Descriptors are accepted for interoperability: `primaryVolumeDescriptors` contains every parsed PVD, while `primaryVolumeDescriptor`, `root`, and `files` use the first by default. Set `primaryVolumeDescriptorIndex` to choose another zero-based PVD, such as `parseIsoImage(image, { primaryVolumeDescriptorIndex: 1 })`. For legacy images with nonzero unused PVD bytes at BP 89-120, opt in with `allowNonzeroPrimaryVolumeDescriptorUnusedBytes: true`; `validateIsoImage(image)` continues to report those bytes as validation issues. `interoperability: true` includes that PVD tolerance and selects a valid primary tree while retaining nonconformant supplementary or enhanced descriptors as unpopulated metadata; strict parsing remains the default.
